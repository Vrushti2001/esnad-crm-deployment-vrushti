...
function monitorStageAndAttachModal(formCtx) {
    const modalStageIds = ["3b5a344f-9f9d-466b-aa08-611e60964b46"]; // same as your config

    // Helpers for EN+AR UI strings/selectors
    const NEXT_STAGE_SELECTORS = [
        'button[aria-label="Next Stage"]',
        'button[title="Next Stage"]',
        'button[aria-label="المرحلة التالية"]',
        'button[title="المرحلة التالية"]',
        // Fallbacks that often exist in UCI:
        'button[data-id="stageAdvanceAction"]',
        'button[data-id="process-next-stage"]'
    ];

    function findNextStageButton(doc) {
        for (const sel of NEXT_STAGE_SELECTORS) {
            const btn = doc.querySelector(sel);
            if (btn) return btn;
        }
        // very defensive fallback: look for visible button containing EN/AR text
        const buttons = Array.from(doc.querySelectorAll('button'));
        return buttons.find(b => {
            const t = (b.innerText || b.textContent || "").trim();
            return /next/i.test(t) || t.includes("التالي") || t.includes("المرحلة التالية");
        }) || null;
    }
